...
contract ERC721Mintable is ERC721Enumerable ,Ownable{
    ...
    // dogPower: 0x646f67506f776572
    // dogBasePower: 0x646f6742617365506f776572
    // dogExperience: 0x646f67457870657269656e6365
    // dogRecycleTimestamp: 0x646f6752656379636c6554696d657374616d70
    // dogInCycleBurden: 0x646f67496e4379636c6542757264656e
    mapping(uint256 => mapping (bytes32 => uint256)) internal dogConfig;
    // id => creators
    ...
    function setBatchURI(string calldata _uri, uint256[] calldata _id) external onlyOwner {
    ...
    function setMultipleBatchURI(string[] calldata _uri, uint256[] calldata _id) external onlyOwner {
    ...
    function safeMint(uint256 _id, address _to, string calldata _uri) public onlyOwner {
    ...
    function safeBatchMint(uint256[] calldata _id, address[] calldata _to, string calldata _uri) external onlyOwner{
    ...
    function safeMultipleBatchMint(uint256[] calldata _id, address[] calldata _to, string[] calldata _uri) external onlyOwner {
        require(_id.length == _to.length, "_ids and _to array length must match.");
        require(_id.length == _uri.length, "_ids and _uri array length must match.");
        for (uint256 i = 0; i < _to.length; ++i) {
    ...
    // dogRecycleTimestamp: 0x646f6752656379636c6554696d657374616d70
    // dogInCycleBurden: 0x646f67496e4379636c6542757264656e
    // dogChampionTime: 0x646f674368616d70696f6e54696d65
    // dog2ChampionTime: 0x646f67324368616d70696f6e54696d65
    // dog3ChampionTime: 0x646f67334368616d70696f6e54696d65
    
    function setSingleProperty(bytes32 _property, uint256 _id, uint256 _value) external onlyManager {
        dogConfig[_id][_property] = _value;
    }
    
    function setMultipleProperty(bytes32 _property, uint256[] calldata _id, uint256[] calldata _value) external onlyManager {
        //require(msg.sender == creators[_id]);
        for (uint256 i = 0; i < _id.length; ++i) {
            dogConfig[_id[i]][_property] = _value[i];
        }
    }
    ...
    function getBatchProperty(bytes32[] calldata _property, uint256[] calldata _id) public view returns (uint256[] memory _value) {
        ...
        //require(msg.sender == creators[_id]);
        for (uint256 i = 0; i < _id.length; ++i) {
            K[i] = dogConfig[_id[i]][_property[i]];
        }
